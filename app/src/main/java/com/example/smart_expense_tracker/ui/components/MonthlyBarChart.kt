...
fun MonthlyBarChart(
    ...
    val labelsToShow = when {
        ...
        else -> 7 // Max 7 labels for a month view
    }
    ...
    val indicesToShow = remember(data.size, labelsToShow) {
        if (data.size <= labelsToShow) {
            data.indices.toSet()
        } else {
            // Guarantees first and last labels are shown
            (0 until labelsToShow).map {
                (it.toFloat() / (labelsToShow - 1) * (data.size - 1)).roundToInt()
            }.toSet()
        }
    }
    ...
    Row(
        ...
        Canvas(
            ...
            (0..gridLineCount).forEach { i ->
                ...
                drawText(textResult, topLeft = Offset(yAxisLabelWidth - textResult.size.width - 4.dp.toPx(), y - textResult.size.height / 2))
            }
            ...
            data.forEachIndexed { index, barData ->
                ...
                val valueTextResult = textMeasurer.measure(text = valueText, style = TextStyle(color = colorScheme.onSurface, fontSize = 10.sp))
                if (barData.value > 0) {
                    drawText(valueTextResult, topLeft = Offset(barStartX + barWidth.toPx() / 2 - valueTextResult.size.width / 2, barTopY - valueTextResult.size.height - 4.dp.toPx()))
                }

                if (index in indicesToShow) {
                    val labelTextResult = textMeasurer.measure(text = barData.label, style = TextStyle(color = colorScheme.onSurface.copy(alpha = 0.6f), fontSize = 10.sp))
                    drawText(labelTextResult, topLeft = Offset(barStartX + barWidth.toPx() / 2 - labelTextResult.size.width / 2, canvasChartHeight + 8.dp.toPx()))
                }
            }
        }
    }
